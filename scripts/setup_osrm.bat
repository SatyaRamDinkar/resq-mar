...
curl -L -o sri-lanka-latest.osm.pbf https://download.geofabrik.de/asia/sri-lanka-latest.osm.pbf

:: Step 3: Run OSRM extraction (Docker)
echo [INFO] Running OSRM extraction (this may take 5-10 minutes)...
docker run -t -v "%cd%":/data osrm/osrm-backend osrm-extract -p /opt/car.lua /data/sri-lanka-latest.osm.pbf

:: Step 4: Run OSRM partition
echo [INFO] Partitioning...
docker run -t -v "%cd%":/data osrm/osrm-backend osrm-partition /data/sri-lanka-latest.osrm

:: Step 5: Run OSRM customize
echo [INFO] Customizing...
docker run -t -v "%cd%":/data osrm/osrm-backend osrm-customize /data/sri-lanka-latest.osrm

:: Step 6: Start OSRM server
echo [INFO] Starting OSRM server on port 5000...
echo [OK] OSRM will be available at http://localhost:5000
docker run -d -p 5000:5000 -v "%cd%":/data osrm/osrm-backend osrm-routed --algorithm mld /data/sri-lanka-latest.osrm
